...
class RolesPermissionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        Role::where('name', 'Administrador')->first()->permissions()->attach([
            0 => ['permissionLevel' => 3]
        ]);

        // Recursos Humanos - gestión avanzada de roles (3), consulta permisos (1)
        Role::where('name', 'Recursos Humanos')->first()->permissions()->attach([
            1 => ['permissionLevel' => 3],
        ]);

        // Encargado de Tienda - edición productos (2), consulta existencias (1), consulta reservas (1)
        Role::where('name', 'Encargado de Tienda')->first()->permissions()->attach([
            2 => ['permissionLevel' => 2],
            3 => ['permissionLevel' => 1],
            4 => ['permissionLevel' => 1],
        ]);
        ...
        Role::where('name', 'Social Media Manager')->first()->permissions()->attach([
            5 => ['permissionLevel' => 2],
            4 => ['permissionLevel' => 3]
        ]);

        // Ventas - edición avanzada de productos (3), consulta promociones (1)
        Role::where('name', 'Ventas')->first()->permissions()->attach([
            2 => ['permissionLevel' => 3],
            5 => ['permissionLevel' => 1],
        ]);
    }
